      ...
      <form onSubmit={handleSubmit} className="space-y-5 text-slate-200">
        ...
        <div className="space-y-2">
          ...
          <div className="grid grid-cols-3 gap-2">
            {[
              { id: 'Admin' as UserRole, label: 'Quản trị (Admin)', desc: 'Toàn quyền hệ thống', color: 'border-rose-500/50 bg-rose-500/10 text-rose-300' },
              { id: 'Staff' as UserRole, label: 'Nhân viên (Staff)', desc: 'Chăm sóc nông trại', color: 'border-sky-500/50 bg-sky-500/10 text-sky-300' },
              { id: 'Customer' as UserRole, label: 'Khách hàng', desc: 'Thuê đất & mua sắm', color: 'border-emerald-500/50 bg-emerald-500/10 text-emerald-300' },
            ].map((item) => (
              <button
                key={item.id}
                type="button"
                ...
                className={`p-3 rounded-xl border text-left transition-all ${
                  role === item.id
                    ? `${item.color} ring-2 ring-emerald-500`
                    : 'border-slate-800 bg-slate-900/50 hover:bg-slate-800/80 text-slate-400'
                }`}
              >
                <div className="font-bold text-xs flex items-center justify-between">
                  <span>{item.label}</span>
                  {role === item.id && <CheckCircle className="w-3.5 h-3.5 text-emerald-400 shrink-0" />}
                </div>
        ...
        <div className="space-y-2">
          ...
          <div className="grid grid-cols-3 gap-2">
            {[
              { id: 'ACTIVE' as UserStatus, label: 'Hoạt động', desc: 'Có thể đăng nhập', icon: CheckCircle, color: 'text-emerald-400' },
              { id: 'LOCKED' as UserStatus, label: 'Khóa tài khoản', desc: 'Cấm truy cập', icon: AlertTriangle, color: 'text-rose-400' },
              { id: 'PENDING' as UserStatus, label: 'Chờ kích hoạt', desc: 'Xác minh thông tin', icon: Shield, color: 'text-amber-400' },
            ].map((item) => (
              <button
                key={item.id}
                type="button"
                ...
                className={`p-3 rounded-xl border text-left transition-all ${
                  status === item.id
                    ? 'border-slate-600 bg-slate-800 text-white ring-2 ring-emerald-500'
                    : 'border-slate-800 bg-slate-900/50 hover:bg-slate-800/80 text-slate-400'
                }`}
              >
                <div className="font-bold text-xs flex items-center justify-between">
