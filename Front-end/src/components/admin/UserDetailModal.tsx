...
export const UserDetailModal: React.FC = () => {
  ...
  const getRoleBadge = (role: string) => {
    switch (role) {
      case 'Admin':
        return <Badge variant="danger" icon={<Shield className="w-3 h-3" />}>Quản trị viên (Admin)</Badge>;
      case 'Staff':
        return <Badge variant="info" icon={<User className="w-3 h-3" />}>Nhân viên (Staff)</Badge>;
      default:
        return <Badge variant="neutral" icon={<User className="w-3 h-3" />}>Khách hàng (Customer)</Badge>;
    }
  };
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'ACTIVE':
        return <Badge variant="success" dot>Đang hoạt động</Badge>;
      case 'LOCKED':
        return <Badge variant="danger" dot>Tài khoản bị khóa</Badge>;
      case 'PENDING':
        return <Badge variant="warning" dot>Chờ duyệt</Badge>;
      default:
  ...
  return (
    ...
    >
      <div className="space-y-6 text-slate-200">
        {/* User Card Header */}
        <div className="flex flex-col sm:flex-row items-center sm:items-start gap-4 p-4 rounded-2xl bg-slate-900/80 border border-slate-800">
          <img
          ...
          <div className="space-y-1.5 text-center sm:text-left flex-1">
            <div className="flex flex-wrap items-center justify-center sm:justify-start gap-2">
              <h3 className="text-xl font-extrabold text-white">{selectedUser.fullName}</h3>
              <span className="text-xs font-mono text-slate-400 bg-slate-800 px-2 py-0.5 rounded-md border border-slate-700">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
          <div className="p-3.5 rounded-xl bg-slate-900/60 border border-slate-800 space-y-1">
            ...
            <p className="font-semibold text-slate-100 text-sm break-all">{selectedUser.email}</p>
          </div>
