  ...
  const result = await pool.request()
    ...
    .query(`
      SELECT
        ...
        -- Thống kê yêu cầu chăm sóc đang chờ
        (
          SELECT COUNT(*)
        ...
        -- Lịch hôm nay còn chưa hoàn thành
        (
          SELECT COUNT(*)
      ...
      INNER JOIN Plots        p   ON sa.PlotId      = p.PlotId
      LEFT  JOIN FarmAreas    fa  ON p.AreaId        = fa.AreaId
      LEFT  JOIN Cameras      cam ON p.CameraId      = cam.CameraId
      -- Lấy mùa vụ ĐANG HOẠT ĐỘNG của ô đất (GROWING hoặc PLANTING)
...
const getMySchedules = async (staffId, filters = {}) => {
  ...
  let query = `
    ...
    INNER JOIN StaffAssignments sa ON sa.PlotId = p.PlotId AND sa.StaffId = @StaffId
    WHERE 1 = 1
  `;
...
const completeSchedule = async (staffId, scheduleId, { resultNote, resultImageUrl } = {}) => {
  ...
  const check = await pool.request()
    ...
    .query(`
      ...
      INNER JOIN Cultivations c ON cs.CultivationId = c.CultivationId
      INNER JOIN Plots p        ON c.PlotId = p.PlotId
      LEFT  JOIN StaffAssignments sa ON sa.PlotId = p.PlotId AND sa.StaffId = @StaffId
...
// ─────────────────────────────────────────────────────────────────────────────
// 6. Lấy danh sách yêu cầu chăm sóc Staff cần xử lý
// ─────────────────────────────────────────────────────────────────────────────

/**
 * Lấy tất cả yêu cầu chăm sóc PENDING trên các ô đất Staff đang phụ trách
 *
 * @param {number} staffId
...
const getMyCareRequests = async (staffId, filters = {}) => {
  ...
  let query = `
    ...
    INNER JOIN Seeds s         ON c.SeedId = s.SeedId
    LEFT  JOIN Users u         ON ro.UserId = u.UserId
    -- Chỉ lấy care requests trên ô đất Staff đang phụ trách
    INNER JOIN StaffAssignments sa ON sa.PlotId = p.PlotId AND sa.StaffId = @StaffId
    WHERE 1 = 1
  `;
...
// ─────────────────────────────────────────────────────────────────────────────
// 7. Lấy danh sách đơn thu hoạch Staff cần xử lý
// ─────────────────────────────────────────────────────────────────────────────

/**
 * Lấy danh sách đơn thu hoạch trên các ô đất Staff đang phụ trách
 *
 * @param {number} staffId
 * @param {{ status?: string }} filters
 */
const getMyHarvestOrders = async (staffId, filters = {}) => {
  ...
  let query = `
    ...
    LEFT  JOIN Users u         ON ro.UserId = u.UserId
    LEFT  JOIN Deliveries d    ON d.HarvestRequestId = hr.HarvestRequestId
    -- Chỉ lấy đơn thu hoạch trên ô đất Staff đang phụ trách
    INNER JOIN StaffAssignments sa ON sa.PlotId = p.PlotId AND sa.StaffId = @StaffId
    WHERE 1 = 1
  `;
